...
class SchedulerGenerator:
    ...
    async def generate_schedule(
        ...
        try:
            ...
            sid_base = f"life_companion_gen_{date_str}"
    ...
    async def _call_llm(self, prompt: str, *, sid: str = "life_companion_gen") -> str:
    ...
    def _to_schedule_data(
        ...
        *,
        ...
        if not image_prompt:
            image_prompt = self._build_image_prompt(outfit, schedule)
        return ScheduleData(
            date=date_str,
            ...
            source="manual" if manual_extra else "llm",
        )

    @staticmethod
    def _build_image_prompt(outfit: str, schedule: str) -> str:
        """Provide a usable selfie prompt when a custom LLM omits image_prompt."""
        return (
            "自然真实的今日生活照，人物身份保持一致；"
            f"穿搭参考：{outfit[:260]}；"
            f"场景和活动参考：{schedule[:360]}；"
            "生活化构图，光线自然，细节清晰，不要文字水印。"
        )
